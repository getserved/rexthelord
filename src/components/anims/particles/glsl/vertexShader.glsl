...
const float amount = 36000.0;
const float amountPerDegree = 10.0;
...
/*
    ...
    float radius = 300.0 + floor(i /360.0) * 3.0;
    float toCenter = clamp(radius / 600.0, 0.0, 1.0);

    float group = mod(floor(i / 360.0), 9.0);
    ...
    finalPos.x = pos.x + snoise(basePos.xy * 1. + u_noise_time * .03 ) * 1.0;
    finalPos.y = pos.y + snoise(basePos.xy * 1. + u_noise_time * .03 + 3.0 ) * 1.0;

    v_alpha = toCenter - (sin(pow(toCenter, 1.2) * 60.0) + 1.0) / 2.0 * 0.5;

    finalPos.x /= 1000.;
    finalPos.y /= 1000.;
    //vec3 pos = vec3(sin(angle)  * radius, cos(angle) * radius, 0.0);
...
*/
    ...
    float angle = i / 180.0 * PI;
    //float radius = 300.0 + floor(i / 360.0) * 3.0;
    float radius = 300.0 + floor(i / 360.0) * u_ring_width;
    float toCenter = clamp(radius / 600.0, 0.0, 1.0);
